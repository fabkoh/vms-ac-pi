...
def test_toggleRelay2(mock_relaySetHighLow, mock_setup_cleanup):
    '''
    This functions tests that toggleRelay2 properly toggles the relay pin high,
    then to low, with the correct pin
    '''
    global relayPinNumber, relayPinSetting
    relayPinNumber = 0
    ...
    # ------------- TEST SECTION: Toggle Relay 1, 2 seconds --------------------
    relay.toggleRelay2(5, 2000, 1000)
    assert relayPinSetting == ["High", "Low"]
    assert relayPinNumber == 5
    # ------------- END OF TEST SECTION ----------------------------------------
    ...
    # ------------- TEST SECTION: Toggle Relay 1, 1 second, twice --------------
    relay.toggleRelay2(10, 1000, 1000)
    assert relayPinSetting == ["High", "Low"]
    assert relayPinNumber == 10
    # ------------- END OF TEST SECTION ----------------------------------------
...
def test_lock_unlock_entrance_one(mock_relaySetHighLow, mock_setup_cleanup):
    ...
    # ------------- TEST SECTION: Lock Entrance 1, no TPO ----------------------
    relay.lock_unlock_entrance_one(thirdPartyOption=None, unlock=False)
    ...
    # ------------- TEST SECTION: Unlock Entrance 1, TPO Gen 1 -----------------
    relay.lock_unlock_entrance_one(thirdPartyOption="GEN_OUT_1", unlock=True)
    ...
    # ------------- TEST SECTION: Unlock Entrance 1, TPO Gen 3 -----------------
    relay.lock_unlock_entrance_one(thirdPartyOption="GEN_OUT_3", unlock=True)
