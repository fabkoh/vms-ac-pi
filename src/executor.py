...
class ThreadPoolMonitor:
    ...
    def __init__(self, max_workers):
        ...
        self.monitoring_thread = threading.Thread(
            target=self._monitor_queue_length)
    ...
    def submit(self, func, *args, **kwargs):
        ...
        logger.info(
            f"Task {task_id} submitted: {func.__name__}. Total submitted: {len(self.active_tasks)}"
        )
        future = self.executor.submit(self._run, task_id, func, *args,
                                      **kwargs)
    ...
    def _task_complete(self, task_id, future):
        ...
        logger.info(
            f"Task {task_id} completed. Total remaining: {len(self.active_tasks)}"
        )
